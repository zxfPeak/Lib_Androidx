...
public class CalendarView extends FrameLayout {
    ...
    private void init(Context context) {
        ...
        mDelegate.mInnerListener = new OnInnerDateSelectedListener() {
            ...
            public void onWeekDateSelected(Calendar calendar, boolean isClick) {
                mDelegate.mIndexCalendar = calendar;
                if (mDelegate.getSelectMode() == CalendarViewDelegate.SELECT_MODE_DEFAULT || isClick || mDelegate.mIndexCalendar.equals(mDelegate.mSelectedCalendar)) {
                    mDelegate.mSelectedCalendar = calendar;
                }
                ...
                mMonthPager.updateSelected();
                if (mWeekBar != null && (mDelegate.getSelectMode() == CalendarViewDelegate.SELECT_MODE_DEFAULT || isClick || mDelegate.mIndexCalendar.equals(mDelegate.mSelectedCalendar))) {
                    mWeekBar.onDateSelected(calendar, mDelegate.getWeekStart(), isClick);
                }
            }
        };
    ...
    public void setRange(int minYear, int minYearMonth, int maxYear, int maxYearMonth) {
        setRange(minYear, minYearMonth, 0, maxYear, maxYearMonth, -1);
    ...
    private void closeSelectLayout(final int position) {
        ...
        if (position == mMonthPager.getCurrentItem()) {
            if (mDelegate.mDateSelectedListener != null && mDelegate.getSelectMode() != CalendarViewDelegate.SELECT_MODE_SINGLE) {
    ...
    public void scrollToSelectCalendar() {
        ...
        scrollToCalendar(mDelegate.mSelectedCalendar.getYear(), mDelegate.mSelectedCalendar.getMonth(), mDelegate.mSelectedCalendar.getDay(), false);
    }
    ...
    public void setMonthView(Class<?> cls) {
        ...
        if (monthViewClassPath.equals(mDelegate.getMonthViewClass()) && !TextUtils.isEmpty(mDelegate.getMonthViewClass())) {
            return;
        }
    ...
    public void setWeekView(Class<?> cls) {
        ...
        if (weekViewClassPath.equals(mDelegate.getWeekViewClass()) && !TextUtils.isEmpty(mDelegate.getWeekViewClass())) {
            return;
        }
    ...
    public void setWeekBar(Class<?> cls) {
        ...
        if (weekBarClassPath.equals(mDelegate.getWeekBarClass()) && !TextUtils.isEmpty(mDelegate.getWeekBarClass())) {
            return;
        }
