package com.zxf.lib_androidx.customCalendar;/*
...
public class CalendarLayout extends LinearLayout {
    ...
    final void setup(CalendarViewDelegate delegate) {
        ...
        initCalendarPosition(delegate.mSelectedCalendar.isAvailable() ? delegate.mSelectedCalendar : delegate.createCurrentDate());
    ...
    void updateContentViewTranslateY() {
        ...
        if (mDelegate.getMonthViewShowMode() == CalendarViewDelegate.MODE_ALL_MONTH) {
            ...
            mContentViewTranslateY = CalendarUtil.getMonthViewHeight(calendar.getYear(), calendar.getMonth(), mItemHeight, mDelegate.getWeekStart()) - mItemHeight;
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (mYearView == null || mContentView == null || mContentView.getVisibility() != VISIBLE) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                 ...
                 * 2、非滚动控件，直接拦截事件
                 */
                if (dy > 0 && mContentView.getTranslationY() == -mContentViewTranslateY && y >= CalendarUtil.dipToPx(getContext(), 98)) {
                    if (!isScrollTop()) {
                        return false;
                    }
                }
                ...
                if (Math.abs(dy) > mTouchSlop) {//大于mTouchSlop开始拦截事件，ContentView和ViewPager得到CANCEL事件
                    if ((dy > 0 && mContentView.getTranslationY() <= 0) || (dy < 0 && mContentView.getTranslationY() >= -mContentViewTranslateY)) {
                        mLastY = y;
                        return true;
                    }
                }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mContentView != null && mMonthView != null) {
            int h = getHeight() - mItemHeight - (mDelegate != null ? mDelegate.getWeekBarHeight() : CalendarUtil.dipToPx(getContext(), 40)) - CalendarUtil.dipToPx(getContext(), 1);
    ...
    public boolean expand() {
        ...
        ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(mContentView, "translationY", mContentView.getTranslationY(), 0f);
    ...
    public boolean shrink() {
        ...
        ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(mContentView, "translationY", mContentView.getTranslationY(), -mContentViewTranslateY);
        objectAnimator.setDuration(240);
        objectAnimator.addUpdateListener(new AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
    ...
    final void hideContentView() {
        if (mContentView == null)
            return;
        mContentView.animate().translationY(getHeight() - mMonthView.getHeight()).setDuration(220).setInterpolator(new LinearInterpolator()).setListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
                super.onAnimationEnd(animation);
