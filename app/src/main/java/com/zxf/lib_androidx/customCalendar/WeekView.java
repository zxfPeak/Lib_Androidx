...
public abstract class WeekView extends BaseView {
    ...
    public boolean onLongClick(View v) {
        if (mDelegate.mDateLongClickListener == null && mDelegate.mCalendarLongClickListener == null)
            return false;
        if (!isClick) {
            return false;
        }
    ...
    void performClickCalendar(Calendar calendar, boolean isNotice) {
        ...
        if (mDelegate.mDateSelectedListener != null && isNotice && mDelegate.getSelectMode() == CalendarViewDelegate.SELECT_MODE_DEFAULT) {
            mDelegate.mDateSelectedListener.onDateSelected(currentCalendar, false);
        }

        if (mDelegate.mCalendarSelectListener != null && isNotice && mDelegate.getSelectMode() == CalendarViewDelegate.SELECT_MODE_DEFAULT) {
    ...
    void setSelectedCalendar(Calendar calendar) {
        if (mDelegate.getSelectMode() == CalendarViewDelegate.SELECT_MODE_SINGLE && !calendar.equals(mDelegate.mSelectedCalendar)) {
            return;
        }
        mCurrentItem = mItems.indexOf(calendar);
    }
