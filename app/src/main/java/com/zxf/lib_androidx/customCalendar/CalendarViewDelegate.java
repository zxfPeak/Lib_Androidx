...
final class CalendarViewDelegate {
     ...
     * 各种字体颜色，看名字知道对应的地方
     */
    private int mCurDayTextColor, mCurDayLunarTextColor, mWeekTextColor, mSchemeTextColor, mSchemeLunarTextColor, mOtherMonthTextColor, mCurrentMonthTextColor, mSelectedTextColor, mSelectedLunarTextColor, mCurMonthLunarTextColor, mOtherMonthLunarTextColor;
     ...
     * 年视图字体大小
     */
    private int mYearViewMonthTextSize, mYearViewDayTextSize;

    /**
     * 年视图字体和标记颜色
     */
    private int mYearViewMonthTextColor, mYearViewDayTextColor, mYearViewSchemeTextColor;

    /**
     * 星期栏的背景、线的背景、年份背景
     */
    private int mWeekLineBackground, mYearViewBackground, mWeekBackground;
    ...
    private boolean mMonthViewScrollable, mWeekViewScrollable, mYearViewScrollable;

    /**
     * 当前月份和周视图的item位置
     */
    @SuppressWarnings("all") int mCurrentMonthViewItem;
     ...
     * 日期被选中监听
     */
    @Deprecated CalendarView.OnDateSelectedListener mDateSelectedListener;
     ...
     * 外部日期长按事件
     */
    @Deprecated CalendarView.OnDateLongClickListener mDateLongClickListener;
    ...
    final void updateSelectCalendarScheme() {
        if (getSchemeType() == CalendarViewDelegate.SCHEME_TYPE_LIST && mSchemeDate != null && mSchemeDate.size() > 0) {
        ...
        if (getSchemeType() == CalendarViewDelegate.SCHEME_TYPE_MAP && mSchemeDatesMap != null && mSchemeDatesMap.size() > 0) {
