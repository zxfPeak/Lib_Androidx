...
/**
 * Created by zxf on 2019/8/27.
 */
public class WordNavigation extends View {
    ...
    private int space = 10;// 距离右边的间距

    public WordNavigation(Context context) {
        super(context);
        init();
    }

    public WordNavigation(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public WordNavigation(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        bgPaint = new Paint();
        wordsPaint.setColor(0xff0000ff);
        wordsPaint.setTextSize(30);
        wordsPaint.setStrokeWidth(2);
        wordsPaint.setTextAlign(Paint.Align.CENTER);

        bgPaint.setColor(0x40ff0000);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //使得边距好看一些
        int height = getMeasuredHeight() - space;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            //绘制字母
            float wordX = itemWidth / 2 - wordWidth / 2 + space;
